...
function reactor_base:setRodLevelRaw(rawlevel)
	if self.mComponent.setControlRodsLevels and not reactor_base.mAPI2IsBugged then
...
function reactor_base:calculateRodLevelInfo(rawlevel)
	...
	rodLevelRounded = math.floor(rawlevel+0.5)
	...
	return rodLevelRounded, rodLevel, rodExtraLevel
end

function reactor_base:setRodLevelRawAPI1(rawlevel)
	checkArg(1, rawlevel, "number")

	local rodLevelRounded, rodLevel, rodExtraLevel = self:calculateRodLevelInfo(rawlevel)
	local rodLevelState = rodLevel * self.mRodNum + rodExtraLevel
	...
	if self.mRodLevelState ~= rodLevelState then
		...
		self.mRodLevel = rodLevelRounded
...
function reactor_base:setRodLevelRawAPI2(rawlevel)
	...
	if self.mRodLevelState ~= rodLevelState then
		...
		for i=0, rodExtraLevel-1 do
			rodLevelTable[self:getRodIdxFromCenter(i)] = rodLevel+1
		end
		for i=rodExtraLevel, self.mRodNum-1 do
			rodLevelTable[self:getRodIdxFromCenter(i)] = rodLevel
		end

		local retval, error = self.mComponent.setControlRodsLevels(rodLevelTable)
		if retval == nil and error ~= nil then
			reactor_base.mAPI2IsBugged = true
			return self:setRodLevelRawAPI1(rawlevel)
		end
